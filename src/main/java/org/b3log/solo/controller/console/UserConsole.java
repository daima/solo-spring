...
public class UserConsole {
    ...
    public void updateUser(final HttpServletRequest request, final HttpServletResponse response)
            throws Exception {
     ...
     *                 "userRole": "", // optional, uses {@value org.b3log.solo.frame.model.Role#DEFAULT_ROLE} instead if not specified
     *                 "userAvatar": "" // optional
     * @param response the specified http servlet response
     * @param context  the specified http request context
     * @throws Exception exception
     ...
     * @throws Exception exception
     */
    @RequestMapping(value = "/console/users/*/*/*"/* Requests.PAGINATION_PATH_PATTERN */, method=RequestMethod.GET)
    public void getUsers(final HttpServletRequest request, final HttpServletResponse response)
            throws Exception {
        final JSONRenderer renderer = new JSONRenderer();
        if (!userQueryService.isAdminLoggedIn(request)) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
    ...
    public void changeUserRole(final HttpServletRequest request, final HttpServletResponse response)
            throws Exception {
        if (!userQueryService.isAdminLoggedIn(request)) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
            return;
