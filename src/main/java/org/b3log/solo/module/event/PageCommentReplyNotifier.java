...
public final class PageCommentReplyNotifier {
	...
	public void action(final Event<JSONObject> event) throws EventException {
		...
		try {
			...
			final String mailSubject = replyNotificationTemplate.getString("subject").replace("${blogTitle}",
					blogTitle);
			...
			final String mailBody = replyNotificationTemplate.getString("body").replace("${postLink}", pageLink)
					.replace("${postTitle}", pageTitle).replace("${replier}", commenter)
