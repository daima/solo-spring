...
final public class Reflections {
	...
	/**
	 * the maxFindLength to get the 'this' keyword when resolving the
	 * vaibleNames.
	 */
	...
	public static String[] getMethodVariableNames(final Class<?> clazz, final String targetMethodName,
		...
		while (!"this".equals(variableName)) {
			...
			if (j > MAX_FIND_LENGTH) {
				logger.warn("Maybe resolve to VariableNames error [class=" + clazz.getName() + ", targetMethodName="
						+ targetMethodName + ']');
				ifkill = true;
				break;
			}
	...
	public static Set<Method> getInheritedMethods(final Class<?> clazz) {
		final Method[] methods = clazz.getDeclaredMethods();
		final Set<Method> declaredMethodSet = CollectionUtils.arrayToSet(methods);
		final Set<Method> ret = new HashSet<>(declaredMethodSet);
	...
	public static Method getMatch(final Set<Method> methods, final Method maybeSuperclassMethod,
	...
	public static Method getOverrideMethod(final Method superclassMethod, final Class<?> subclass) {
		...
		while (!currentClass.equals(superclass)) {
			try {
				final Method m = currentClass.getDeclaredMethod(superclassMethod.getName(),
